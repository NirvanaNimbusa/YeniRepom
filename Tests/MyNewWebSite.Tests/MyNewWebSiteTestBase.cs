...
{
    public abstract class MyNewWebSiteTestBase : AbpIntegratedTestBase<MyNewWebSiteTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MyNewWebSiteTestBase()
        {
        ...
        protected void UsingDbContext(Action<MyNewWebSiteDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyNewWebSiteDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyNewWebSiteDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyNewWebSiteDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyNewWebSiteDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyNewWebSiteDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyNewWebSiteDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyNewWebSiteDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyNewWebSiteDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyNewWebSiteDbContext, Task<T>> func)
        {
            T result;
